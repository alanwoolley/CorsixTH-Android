...
public class EasyTracker {
	...
	private void loadParameters() {
		...
		if (gaAccountId != null) {
			...
			autoActivityTracking = parameterFetcher
					.getBoolean("ga_auto_activity_tracking");
	 ...
	 * 
	 * @param ctx
	 *            the Context to use to fetch the applicationContext
	 */
	 ...
	 * 
	 * @param activity
	 *            the Activity that is to be tracked
	 */
	public void trackActivityStart(final Activity activity) {
		...
		queueToTrackerThreadIfEnabled(new Runnable() {
			...
			public void run() {
				if (startASession) {
					getGoogleAnalyticsTracker().startNewSession(gaAccountId,
							gaDispatchPeriod, gaContext);

					try {
						String ver = activity.getPackageManager()
								.getPackageInfo(activity.getPackageName(), 0).versionName;
						Log.d(getClass().getSimpleName(), "Logging ver: " + ver);
						getGoogleAnalyticsTracker().setCustomVar(1,
								"App Version", ver, 2);
				...
				if (autoActivityTracking) {
					getGoogleAnalyticsTracker().trackPageView(
	 ...
	 * 
	 * @param referrer
	 *            the campaign referral information to set
	 */
	...
	public void trackEvent(final String category, final String action,
		...
		queueToTrackerThreadIfEnabled(new Runnable() {
			...
			@Override
			public void run() {
				getGoogleAnalyticsTracker().trackEvent(category, action, label,
						value);
			}
